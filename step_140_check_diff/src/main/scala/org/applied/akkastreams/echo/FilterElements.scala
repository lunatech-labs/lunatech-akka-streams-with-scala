...
object FilterElements {
  ...
  object ChkEchoCancel {
    def apply(filter1: Flow[Double, Double, NotUsed], filter2: Flow[Double, Double, NotUsed]) = {
      Flow.fromGraph(GraphDSL.create() { implicit b =>
        import GraphDSL.Implicits._
        ...
        // input                    ~> bcast.in
        bcast                       ~> zip.in0
        bcast ~> filter1 ~> filter2 ~> zip.in1
        zip.out ~> diff
        // diff.out ~> output

        FlowShape[Double, Double](bcast.in, diff.out)
      })
    }
  }
